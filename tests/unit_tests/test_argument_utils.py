...
from megatron.training.argument_utils import (
    ...
    _resolve_hybrid_attention_variant,
...
class TestDsv4HybridCsaCompressRatioNormalization:
    """Test the DSv4 HybridModel compression-ratio normalization contract."""
    ...
    def test_normalizes_default_compact_and_padded_ratios(self, provided, expected_config_ratios):
        ...
        _normalize_dsv4_hybrid_csa_compress_ratios(args, config_kwargs, "-W|EC/H-")
    ...
    def test_ordinary_d_does_not_consume_a_dsv4_ratio(self):
        ...
        assert args.csa_compress_ratios == [4]
        assert config_kwargs['csa_compress_ratios'] == [0, 0, 4, 0]


class TestHybridAttentionVariantResolution:
    """Test that C/H/W layer symbols select only the DSv4 hybrid variant."""

    @pytest.mark.parametrize("symbol", ["C", "H", "W"])
    @pytest.mark.parametrize("variant", ["dsa", "gated_delta_net"])
    def test_rejects_explicit_non_dsv4_variant(self, symbol, variant):
        args = Namespace(experimental_attention_variant=variant)
        config_kwargs = {"experimental_attention_variant": variant}

        with pytest.raises(ValueError, match=r"C/H/W.*dsv4_hybrid"):
            _resolve_hybrid_attention_variant(args, config_kwargs, f"E{symbol}-")

    @pytest.mark.parametrize("variant", [None, "dsv4_hybrid"])
    def test_resolves_dsv4_variant(self, variant):
        args = Namespace(experimental_attention_variant=variant)
        config_kwargs = {"experimental_attention_variant": variant}

        _resolve_hybrid_attention_variant(args, config_kwargs, "-W|EC/H-")

        assert config_kwargs["experimental_attention_variant"] == "dsv4_hybrid"

    @pytest.mark.parametrize("variant", [None, "dsa"])
    def test_resolves_dsa_for_d_only_pattern(self, variant):
        args = Namespace(experimental_attention_variant=variant)
        config_kwargs = {"experimental_attention_variant": variant}

        _resolve_hybrid_attention_variant(args, config_kwargs, "D-E/D")

        assert config_kwargs["experimental_attention_variant"] == "dsa"

    @pytest.mark.parametrize("variant", ["dsv4_hybrid", "gated_delta_net"])
    def test_rejects_explicit_non_dsa_variant_for_d_only_pattern(self, variant):
        args = Namespace(experimental_attention_variant=variant)
        config_kwargs = {"experimental_attention_variant": variant}

        with pytest.raises(ValueError, match=r"D attention.*variant='dsa'"):
            _resolve_hybrid_attention_variant(args, config_kwargs, "D-E/D")
