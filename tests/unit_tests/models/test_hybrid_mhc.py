...
class _StubTransformerLayer(TransformerLayer):
    ...
    def __init__(self, config: TransformerConfig, layer_number: int = 1, **_kwargs):
...
def _get_transformer_submodules() -> HybridStackSubmodules:
    return HybridStackSubmodules(attention_layer=ModuleSpec(module=_StubTransformerLayer))
...
class TestHybridStackMHC:
    ...
    def test_full_recompute_forwards_hash_inputs_to_wrapped_transformer(self, monkeypatch):
        config = _get_config(
            num_layers=2,
            recompute_granularity="full",
            recompute_method="uniform",
        ...
        stack = HybridStack(
            config=config,
            submodules=_get_transformer_submodules(),
            pre_process=True,
            post_process=False,
            post_layer_norm=False,
            layer_type_list=[Symbols.ATTENTION] * 2,
            pg_collection=_get_pg_collection(),
        ).cuda()
        hidden_states = torch.randn(8, 2, config.hidden_size, device="cuda", requires_grad=True)
        ...
        def record_hash_inputs(
            ...
            seen_hash_inputs.append((padding_mask, input_ids))
            return original(
                layer,
                hidden_states,
                attention_mask,
        ...
        output = stack(
            hidden_states, attention_mask=None, padding_mask=padding_mask, input_ids=input_ids
        )
        output.float().sum().backward()

        assert len(seen_hash_inputs) == 4
        assert all(
            captured_padding is not None
            and torch.equal(captured_padding, padding_mask)
            and captured_ids is not None
            and torch.equal(captured_ids, input_ids)
            for captured_padding, captured_ids in seen_hash_inputs
        )
