...
def test_hybrid_hash_moe_rejects_pp_without_token_transport():
    with pytest.raises(ValueError, match="token IDs are not transported between pipeline stages"):
        TransformerConfig(
            num_layers=4,
            hidden_size=64,
...
def test_non_hybrid_hash_moe_pp_still_requires_explicit_pipeline_layout():
...
@pytest.mark.parametrize("chunk_field", ["mlp_chunks_for_training", "mlp_chunks_for_prefill"])
def test_hash_moe_rejects_mlp_chunking(chunk_field):
    with pytest.raises(ValueError, match="Hash routing does not support mlp_chunks"):
        TransformerConfig(
            num_layers=4,
            hidden_size=64,
            num_attention_heads=4,
