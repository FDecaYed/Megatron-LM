...
@pytest.mark.parametrize(
    ("hybrid_layer_pattern", "csa_compress_ratios", "csa_dense_mode", "expected"),
    [
        ("WCDH/CD", [4, 4, 4], False, 4),
        ("WCDH/CD", [4, 4, 4], True, 2),
        ("WCH/WC/WC", [0, 0], False, 3),
        (None, [0, 4, 128, 4], False, 2),
        (None, [0, 4, 128, 4], True, 0),
        (None, None, False, None),
    ],
)
def test_num_indexer_layers_counts_dsa_csa_and_mtp(
    hybrid_layer_pattern, csa_compress_ratios, csa_dense_mode, expected
):
    """The resolved D+C pattern, including MTP depths, is the authoritative denominator."""
    assert (
        get_num_indexer_layers(hybrid_layer_pattern, csa_compress_ratios, csa_dense_mode)
        == expected
    )
