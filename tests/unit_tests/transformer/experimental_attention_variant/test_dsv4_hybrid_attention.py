...
def _make_attention_spec(config):
    ...
    from megatron.core.models.hybrid.dsv4_module_specs import (
...
def test_gpt_dispatcher_does_not_expose_dsv4_hybrid():
    """DSv4 belongs to HybridModel and must not be constructible through GPTModel."""
    from megatron.core.models.gpt.experimental_attention_variant_module_specs import (
        get_experimental_attention_variant_module_spec,
    )

    with pytest.raises(ValueError, match="Invalid experimental attention variant"):
        get_experimental_attention_variant_module_spec(_make_config(), backend=object())
...
@pytest.mark.parametrize(
    ("config_overrides", "error_match"),
    [
        ({"csa_window_size": 0}, "csa_window_size must be positive"),
        ({"csa_window_size": None}, "csa_window_size must be positive"),
        ({"dsa_indexer_topk_freq": 0}, "dsa_indexer_topk_freq must be positive"),
        ({"dsa_indexer_topk_freq": None}, "dsa_indexer_topk_freq must be positive"),
        ({"dsa_indexer_skip_topk_offset": -1}, "dsa_indexer_skip_topk_offset must be non-negative"),
        (
            {"dsa_indexer_skip_topk_offset": None},
            "dsa_indexer_skip_topk_offset must be non-negative",
        ),
        ({"bf16": False}, "requires BF16"),
        ({"add_bias_linear": True}, "does not support linear biases"),
        ({"add_qkv_bias": True}, "does not support linear biases"),
        ({"attention_dropout": 0.1}, "does not implement attention dropout"),
        ({"rotary_percent": 0.5}, "requires full RoPE"),
        ({"qk_l2_norm": True}, "qk_l2_norm is not supported"),
        ({"dsa_indexer_n_heads": None}, "dsa_indexer_n_heads must be positive"),
        ({"dsa_indexer_n_heads": 0}, "dsa_indexer_n_heads must be positive"),
        ({"dsa_indexer_head_dim": None}, "dsa_indexer_head_dim must be positive"),
        ({"dsa_indexer_head_dim": 0}, "dsa_indexer_head_dim must be positive"),
        ({"dsa_indexer_topk": None}, "dsa_indexer_topk must be positive"),
        ({"dsa_indexer_topk": 0}, "dsa_indexer_topk must be positive"),
        ({"dsa_indexer_head_dim": 16}, "dsa_indexer_head_dim must be at least qk_pos_emb_head_dim"),
    ],
)
def test_config_rejects_unsupported_native_dsv4_settings(config_overrides, error_match):
    """Native DSv4 constraints should fail during config construction."""
    with pytest.raises(ValueError, match=error_match):
        _make_config(**config_overrides)


@pytest.mark.parametrize("dense_mode", [False, True])
def test_config_requires_indexer_fields_only_when_ratio4_uses_indexer(dense_mode):
    """Window/HCA-only and dense-oracle runs do not instantiate the ratio-4 indexer."""
    ratios = [0, 128, 0, 128] if not dense_mode else [0, 4, 128, 4]
    config = _make_config(
        csa_compress_ratios=ratios,
        csa_dense_mode=dense_mode,
        dsa_indexer_n_heads=None,
        dsa_indexer_head_dim=None,
        dsa_indexer_topk=None,
        dsa_indexer_topk_freq=0,
        dsa_indexer_skip_topk_offset=-1,
    )
    assert config.csa_dense_mode is dense_mode
...
def test_dense_c_with_ordinary_d_still_requires_indexer_fields(config_overrides, error_match):
    """Dense C disables only its own indexer; an ordinary D in the pattern still owns one."""
    from megatron.training.argument_utils import _count_hybrid_dsa_indexer_layers

    pattern = "CD/HD"
    ordinary_d_count = _count_hybrid_dsa_indexer_layers(pattern)
    assert ordinary_d_count == 2

    with pytest.raises(ValueError, match=error_match):
        _make_config(
            csa_compress_ratios=[4, 0, 128, 0],
            csa_dense_mode=True,
            dsa_indexer_layer_count=ordinary_d_count,
            **config_overrides,
        )
