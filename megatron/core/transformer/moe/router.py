...
class TopKRouter(Router):
    ...
    def _hash_routing(self, logits: torch.Tensor, input_ids: torch.Tensor):
        ...
        assert (
            input_ids.dim() == 2
        ), f"input_ids must have shape [batch, sequence], but got {tuple(input_ids.shape)}."

        # Hidden states are flattened from [sequence, batch, hidden], whereas
        # model token IDs arrive as [batch, sequence]. Under sequence parallelism,
        # hidden states are sharded along sequence before routing, so select the
        # matching sequence shard using this router's explicit TP process group.
        flat_ids = input_ids.T.reshape(-1)
        if self.config.sequence_parallel:
            tp_size = torch.distributed.get_world_size(group=self.tp_group)
            if tp_size > 1:
                assert input_ids.shape[1] % tp_size == 0, (
                    f"input_ids sequence length {input_ids.shape[1]} must be divisible by "
                    f"the tensor parallel size {tp_size}."
                )
                assert flat_ids.numel() == logits.shape[0] * tp_size, (
                    f"input_ids contains {flat_ids.numel()} pre-shard tokens, but router logits "
                    f"contain {logits.shape[0]} local tokens across TP size {tp_size}."
                )
                tp_rank = torch.distributed.get_rank(group=self.tp_group)
                input_ids = input_ids.chunk(tp_size, dim=1)[tp_rank]
                flat_ids = input_ids.T.reshape(-1)
    ...
    def routing(
        ...
        # Match a full-sequence padding mask to the sequence-parallel hidden-state shard,
        # then flatten it in the same [sequence, batch] order as router logits.
        if padding_mask is not None:
            assert padding_mask.dim() == 2, (
                f"padding_mask must have shape [sequence, batch], but got "
                f"{tuple(padding_mask.shape)}."
            )
            if self.config.sequence_parallel:
                tp_size = torch.distributed.get_world_size(group=self.tp_group)
                global_shape = (seq_length * tp_size, bsz)
                if tp_size > 1 and padding_mask.shape == global_shape:
                    tp_rank = torch.distributed.get_rank(group=self.tp_group)
                    padding_mask = padding_mask.chunk(tp_size, dim=0)[tp_rank]
            assert padding_mask.shape == (seq_length, bsz), (
                f"padding_mask has shape {tuple(padding_mask.shape)}, but router logits have "
                f"local shape [{seq_length}, {bsz}]"
                + (
                    f" and sequence parallel routing accepts pre-shard shape {list(global_shape)}."
                    if self.config.sequence_parallel
                    else "."
                )
            )
