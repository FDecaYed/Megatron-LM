...
def get_num_indexer_layers(
    hybrid_layer_pattern: Optional[str],
    csa_compress_ratios: Optional[Sequence[int]],
    csa_dense_mode: bool = False,
) -> Optional[int]:
    """Return the full-model indexer count used as the logging denominator.

    A resolved hybrid pattern contains main and MTP layers. Both ordinary DSA
    (``D``) and non-dense DSv4 CSA (``C``) own an indexer, so count the active
    symbols across the complete pattern, including every section after ``/``.
    Dense CSA is an oracle path without an indexer. The ratio list is retained
    only as a compatibility fallback for pre-HybridModel callers.
    """
    indexer_symbols = ("D",) if csa_dense_mode else ("D", "C")
    if hybrid_layer_pattern is not None:
        return sum(symbol in indexer_symbols for symbol in hybrid_layer_pattern)
    if csa_compress_ratios is not None:
        if csa_dense_mode:
            return 0
        return sum(ratio == 4 for ratio in csa_compress_ratios)
    return None
