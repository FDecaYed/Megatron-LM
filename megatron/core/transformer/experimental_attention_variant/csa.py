...
class CompressedSparseAttention(MegatronModule):
    ...
    * ``ratio == 128``: window + 128x compressed, attend to all (compressor built only)
    ...
    def __init__(
        ...
        self.use_fused_kernels = dsa_kernels.use_fused_dsa_kernels(config)
    ...
    def _select_inference_compressed_topk(
        self,
        ...
        """Select compressed indices, using the shared fused hook when eligible."""
        effective_topk = min(self.indexer.index_topk, k_indexer.size(0))
        scaled_weights = (weights_indexer.float() * self.indexer.softmax_scale).to(
            weights_indexer.dtype
        )
        fused_topk = None
        if self._can_use_fused_indexer_topk(q_indexer, k_indexer, scaled_weights):
            ends = (positions.squeeze(-1) // self.compress_ratio).clamp(max=k_indexer.size(0))
            ends = ends.to(dtype=torch.int32)
        ...
        _, topk_indices = fused_qk_topk_naive(
            q_indexer, k_indexer, scaled_weights, effective_topk, mask=causal_mask
        )
        return topk_indices

    def _can_use_fused_indexer_topk(
        self, q_indexer: torch.Tensor, k_indexer: torch.Tensor, weights_indexer: torch.Tensor
    ) -> bool:
        """Return whether the shared cuDNN indexer hook can be attempted safely."""
        return bool(
            self.use_fused_kernels
            and q_indexer.is_cuda
            and k_indexer.is_cuda
            and weights_indexer.is_cuda
            and q_indexer.dtype == torch.bfloat16
            and k_indexer.dtype == torch.bfloat16
            and weights_indexer.dtype == torch.bfloat16
        )

    def _try_fused_sparse_attention(
        self, query: torch.Tensor, kv_full: torch.Tensor, topk_indices: torch.Tensor
    ) -> Optional[torch.Tensor]:
        """Run shared fused sparse attention for the production CSA layout, or decline."""
        if (
            not self.use_fused_kernels
            or self.v_head_dim != 512
            or query.ndim != 4
            or kv_full.ndim != 3
            or topk_indices.ndim != 3
            or not query.is_cuda
            or not kv_full.is_cuda
            or not topk_indices.is_cuda
            or query.dtype != torch.bfloat16
            or kv_full.dtype != torch.bfloat16
            or query.size(-1) != self.v_head_dim
            or kv_full.size(-1) != self.v_head_dim
        ):
            return None

        key = kv_full.unsqueeze(2)
        attn_sink = self.attn_sink.float()
        if not dsa_kernels.supports_fused_absorbed_sparse_attention(
            self.config, query, key, topk_indices, self.v_head_dim, attn_sink=attn_sink
        ):
            return None
        return dsa_kernels.run_fused_absorbed_sparse_attention(
            self.config,
            query,
            key,
            topk_indices,
    ...
    def forward(
        ...
        if self.compress_ratio > 1 and n_compressed > 0:
            ...
            if self.indexer is not None:
                ...
                if self.training and torch.is_grad_enabled():
                    ...
                    topk_indices_compressed = self._select_inference_compressed_topk(
                        q_indexer, k_indexer, weights_indexer, causal_mask, positions
                    )
        ...
        fused_output = self._try_fused_sparse_attention(query, kv_full, topk_idxs)
        if fused_output is None:
            output = unfused_compressed_sparse_attn(
                query, kv_full, self.attn_sink.float(), topk_idxs, self.softmax_scale
            )
        else:
            output = fused_output.reshape(sq, b, np * hn)
