...
def _validate_dsv4_hybrid_layer_specs(
    layer_type_list: List[str], submodules: HybridStackSubmodules
) -> None:
    """Reject DSv4 layer symbols that still resolve to an identity layer."""
    slots = (
        (LayerSymbols.CSA, "csa_layer"),
        (LayerSymbols.HCA, "hca_layer"),
        (LayerSymbols.WINDOW, "window_layer"),
    )
    for layer_symbol, slot_name in slots:
        if (
            layer_symbol in layer_type_list
            and get_module(getattr(submodules, slot_name)) is IdentityOp
        ):
            raise ValueError(
                f"Hybrid layer symbol {layer_symbol!r} requires a concrete "
                f"HybridStackSubmodules.{slot_name} spec; the resolved slot is IdentityOp."
            )
...
class HybridStack(MegatronModule):
    ...
    def __init__(
        ...
        _validate_dsv4_hybrid_layer_specs(layer_type_list, submodules)
